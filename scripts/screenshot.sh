...
if ! command -v grim &>/dev/null || ! command -v slurp &>/dev/null; then
  notify-send -h boolean:transient:true "Screenshot Failed" "Required tools (grim or slurp) are not installed." -i dialog-error
  exit 1
fi
...
if [[ -z "$REGION" ]]; then
  notify-send -h boolean:transient:true "Screenshot Canceled" "No region selected." -i dialog-warning
  exit 1
fi
...
if [[ -f "$FILE" && -s "$FILE" ]]; then
  notify-send -h boolean:transient:true "Screenshot Taken" "Saved to $FILE\nRegion: $REGION" -i camera
  exit 0
else
  notify-send -h boolean:transient:true "Screenshot Failed" "Could not save the screenshot." -i dialog-error
  exit 1
fi
